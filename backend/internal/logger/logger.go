...
func NewLoggerService(cfg *config.ObservabilityConfig) *LoggerService {
	...
	service.nrApp = app
	fmt.Println("New Relic application initialized successfully for app: ", cfg.ServiceName)
	return service
}
...
// because in go, if in a struct a field starts with lowercase, it is unexported and cannot be accessed outside the package. So we need to provide a getter method to access the New Relic application instance from other packages if needed.
...
func NewLoggerWithService(cfg *config.ObservabilityConfig, loggerService *LoggerService) zerolog.Logger {
	...
	// in production we use new relic and simply write to stdout, in development we use console writer for better readability
	// Setup base writer
