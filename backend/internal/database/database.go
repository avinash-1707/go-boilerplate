...
func New(cfg *config.Config, logger *zerolog.Logger, loggerService *loggerConfig.LoggerService) (*Database, error) {
	...
	// here dsn means data source name, it is a string that contains the information needed to connect to the database. It typically includes the username, password, host, port, database name, and any additional parameters required for the connection. The format of the DSN can vary depending on the database driver being used. In this case, we are constructing a DSN for a PostgreSQL database using the pgx driver.
